...
def plot_binomial_path(path: list, output_path: Path):
    """Plot binomial tree path with Tufte style."""
    setup_tufte_style()
    fig, ax = plt.subplots(figsize=(10, 4))
    ax.plot(path, color="#4A90A4", linewidth=1.2)
    apply_tufte_style(ax, title="Binomial Model Path under Risk-Neutral Measure")
    ax.set_xlabel("Step")
    ax.set_ylabel("Price")
...
def plot_exponential_martingale(t: np.ndarray, Z: np.ndarray, output_path: Path):
    """Plot exponential martingale with Tufte style."""
    setup_tufte_style()
    fig, ax = plt.subplots(figsize=(10, 4))
    ax.plot(t, Z, color="#4A90A4", linewidth=1.2)
    apply_tufte_style(ax, title="Exponential Martingale from Brownian Motion")
    ax.set_xlabel("Time")
    ax.set_ylabel("Z(t)")
    save_tufte_figure(output_path)
...
def plot_girsanov_transformation(t: np.ndarray, W: np.ndarray, W_tilde: np.ndarray, output_path: Path):
    """Plot Girsanov transformation with Tufte style."""
    setup_tufte_style()
    fig, ax = plt.subplots(figsize=(10, 4))
    ...
    apply_tufte_style(ax, title="Girsanov Change of Measure")
    ax.set_xlabel("Time")
    ax.set_ylabel("Value")
    ax.legend(loc='best')
    save_tufte_figure(output_path)
