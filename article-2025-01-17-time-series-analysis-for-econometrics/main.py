...
from src.core import perform_adf_test, perform_cointegration_test, fit_var_model, plot_time_series
...
def main():
    ...
    if config['analysis']['adf_test']:
        print("Performing ADF tests...")
        for col in data.columns:
            result = perform_adf_test(data[col])
            print(f"{col}: ADF={result['adf_statistic']:.4f}, p-value={result['p_value']:.4f}, stationary={result['is_stationary']}")
    
    if config['analysis']['cointegration_test']:
        print("\nPerforming cointegration test...")
        result = perform_cointegration_test(data['x'], data['y'])
        print(f"Cointegration: test_stat={result['test_statistic']:.4f}, p-value={result['p_value']:.4f}, cointegrated={result['is_cointegrated']}")
