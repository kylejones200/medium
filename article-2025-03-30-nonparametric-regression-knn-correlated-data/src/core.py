...
def plot_time_series(data: pd.DataFrame, target_col: str, output_path: Path):
    """Plot raw time series with Tufte style."""
    setup_tufte_style()
    fig, ax = plt.subplots(figsize=(12, 6))
    
    ax.plot(data.index, data[target_col], color="#4A90A4", linewidth=1.2)
    apply_tufte_style(ax, title='Daily Minimum Temperatures in Melbourne')
    ax.set_xlabel("Date")
...
def plot_knn_predictions(y_test: pd.Series, y_pred: np.ndarray, output_path: Path):
    """Plot KNN predictions vs actual with Tufte style."""
    setup_tufte_style()
    fig, ax = plt.subplots(figsize=(12, 6))
    ...
    apply_tufte_style(ax, title='k-NN Regression: Actual vs Predicted Temperatures')
    ax.set_xlabel("Date")
    ax.set_ylabel("Temperature (°C)")
    ax.legend(loc='best')
...
def plot_kernel_smoothing(data: pd.DataFrame, y_pred: np.ndarray, target_col: str, output_path: Path):
    """Plot kernel smoothing results with Tufte style."""
    setup_tufte_style()
    fig, ax = plt.subplots(figsize=(12, 6))
    ...
    apply_tufte_style(ax, title='Kernel Smoothing of Daily Minimum Temperatures')
    ax.set_xlabel("Date")
    ax.set_ylabel("Temperature (°C)")
    ax.legend(loc='best')
