...
def plot_denoised_data(df_train: pd.DataFrame, output_path: Path):
    """Plot denoised and normalized data with Tufte style."""
    setup_tufte_style()
    ...
    apply_tufte_style(ax, title="Denoised and Normalized Data")
    ax.set_xlabel("Time")
...
def plot_anomaly_detection(df: pd.DataFrame, anomalies: pd.DataFrame, 
    ...
    """Plot anomaly detection results with Tufte style."""
    setup_tufte_style()
    fig, ax = plt.subplots(figsize=(12, 6))
    ...
    apply_tufte_style(ax, title=f"Anomaly Detection in {feature}")
    ax.set_xlabel("Time")
...
def plot_correlation_heatmap(df: pd.DataFrame, output_path: Path):
    """Plot correlation heatmap with Tufte style."""
    setup_tufte_style()
    corr = df.corr()
    ...
    apply_tufte_style(ax, title="Correlation Heatmap")
