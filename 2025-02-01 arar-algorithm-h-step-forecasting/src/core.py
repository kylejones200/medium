...
def plot_forecast_comparison(y: pd.Series, y_forecast_arar: np.ndarray,
    ...
    apply_tufte_style(ax, f"ARAR Algorithm: {h}-Step Forecast")
    ax.set_xlabel("Time")
    ax.set_ylabel("Value")
    ax.legend(loc='best')
...
def plot_arar_vs_arima(y: pd.Series, train: pd.Series, test: pd.Series,
    ...
    """Plot ARAR vs ARIMA forecast comparison with Tufte style."""
    setup_tufte_style()
    fig, ax = plt.subplots(figsize=(12, 6))
