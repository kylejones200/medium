...
def plot_frequency_domain(frequencies: np.ndarray, fft_result: np.ndarray, 
    ...
    """Plot frequency domain representation with Tufte style."""
    setup_tufte_style()
    fig, ax = plt.subplots(figsize=(10, 4))
...
def plot_noise_filtering(time: np.ndarray, noisy_signal: np.ndarray, 
    ...
    """Plot noise filtering comparison with Tufte style."""
    setup_tufte_style()
    ...
    apply_tufte_style(ax, title="Noise Filtering with Fourier Transform")
    ax.set_xlabel("Time")
    ax.set_ylabel("Amplitude")
    ax.legend(loc='best')
