...
logging.info(f"Classification Accuracy: {accuracy:.2f}")
logging.info(f"Regression MAE (RF): {mae_rf:.2f}")
...
if best_model:
    logging.info(f"Best ARIMA model: ARIMA{best_pdq}")
    ...
    logging.info(f"Regression MAE (ARIMA): {mae_arima:.2f}")
    ...
    logging.info("No valid ARIMA model found.")
