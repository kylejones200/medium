...
def plot_cumulative_irf(irf, fitted_model, shock_var: str, response_vars: list,
    ...
    """Plot cumulative impulse response functions with Tufte style."""
    setup_tufte_style()
    ...
    for i, var in enumerate(response_vars):
        ...
        apply_tufte_style(axes[i], title=f'Cumulative IRF: {shock_var} → {var}')
...
def plot_irf(irf, output_path: Path):
    """Plot standard IRF with Tufte style."""
    setup_tufte_style()
    irf.plot(orth=False)
    plt.tight_layout()
